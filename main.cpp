...
bool checkForDebugger() {
  ...
  for (int i = 0; i < 1000; i++) {
    counter++;
  } // Removed volatile to fix warning
...
bool checkTimeConsistency() {
  ...
  double monotonicDiff =
      (currentMonotonic.tv_sec - lastMonotonic.tv_sec) +
  ...
  // Calculate time differences
  double qpcDiff =
      (double)(currentQPC.QuadPart - lastQPC.QuadPart) / qpcFreq.QuadPart;
  double systemTimeDiff = (currentTime.QuadPart - lastTime.QuadPart) /
                          10000000.0; // Convert 100ns to seconds
...
bool checkVirtualMachine() {
  ...
  const char *vmIndicators[] = {"/sys/devices/virtual/dmi/id/product_name",
  ...
  if (dir != NULL) {
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      if (entry->d_type == DT_DIR && isdigit(entry->d_name[0])) {
        ...
        snprintf(cmdlinePath, sizeof(cmdlinePath), "/proc/%s/cmdline",
                 entry->d_name);
  ...
  char buffer[1024];
  DWORD size = GetSystemFirmwareTable(
      'RSMB', 0, buffer, sizeof(buffer)); // Fixed multi-character constant
  if (size > 0) {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Services", 0,
    ...
    while (RegEnumKeyEx(hKey, index, subKeyName, &nameSize, NULL, NULL, NULL,
                        NULL) == ERROR_SUCCESS) {
